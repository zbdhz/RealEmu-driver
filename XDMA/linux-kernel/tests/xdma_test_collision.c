...
int main() {
    ...
    uint8_t *tx_buf1 = (uint8_t*)aligned_alloc(4096, buf_size); // 改为uint8_t类型
    uint8_t *tx_buf2 = (uint8_t*)aligned_alloc(4096, buf_size); // 改为uint8_t类型
    ...
    MacBridge_TOP bridge_data1 = {
        ...
        .macEvent = {
            .srcMacId = 1,   // 源MAC ID //74
            .dstMacId = 0,   // 目标MAC ID设为1 //84
            .rfParam = {
                .power = 1000,  // 功率设为最大值 //98
                .mcs = 0       // MCS设为0 //110
            },
            .mpduDigest = {
                .frametype = 2,     // 帧w类型
    ...
    MacBridge_TOP bridge_data2 = {
        ...
        .macEvent = {
            .srcMacId = 2,   // 源MAC ID //74
            .dstMacId = 0,   // 目标MAC ID设为1 //84
            .rfParam = {
                .power = 2000,  // 功率设为最大值 //98
                .mcs = 0       // MCS设为0 //110
            },
            .mpduDigest = {
                .frametype = 2,     // 帧w类型
    ...
    memcpy(tx_buf1, &buffer1, sizeof(buffer1)); // 将数据复制到tx_buf

    uint8_t buffer2[BUFFER_SIZE] = {0};
    direct_reverse_mac_bridge_to_buffer(&bridge_data2, buffer2);
    memcpy(tx_buf2, &buffer2, sizeof(buffer2)); // 将数据复制到tx_buf
