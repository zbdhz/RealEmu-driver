...
int main() {
    ...
    MacBridge_TOP bridge_data = {
        ...
        .macEvent = {
            .srcMacId = 0,   // 源MAC ID //74
            .dstMacId = 1,   // 目标MAC ID设为1 //84
            .rfParam = {
    ...
    // memcpy(tx_buf, &macevent, sizeof(MacEvent)); // 将数据复制到tx_buf
    // memcpy(tx_buf, &bridge_data, sizeof(MacBridge_TOP)); // 将数据复制到tx_buf
    // printf("sizeof(MacEvent): %ld",sizeof(MacEvent));
    // memcpy(tx_buf, &cfg_bridge_data, sizeof(CfgBridge_TOP)); // 将数据复制到tx_buf
    // 确保结构体大小与传输大小匹配
	...
	if (fork() == 0) {
        ...
        int timeout_count = 0;
        const int MAX_TIMEOUTS = 2;  // 最大超时次数
